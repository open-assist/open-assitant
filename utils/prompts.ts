...
Code Interpreter allows the Assistant to write and run code in a sandboxed execution environment.
These tools can process files with diverse data and formatting, and generate files with data and images
of graphs.
...
<name>execute</name>
<description>
When you send a message containing code to \`execute\` function, it will be executed in a stateful
Jupyter notebook environment. \`execute\` function will respond with the output of the execution or
...
The drive at '/mnt/data' can be used to save and persist users' files. Internet access for this session
is disabled. Do not make external web requests or API calls as they will fail.`;

export const FUNCTION_TOOLS_PROMPT = `\n\n## Function Tools

Function tools allow users to describe functions to Assistant and have it intelligently return the
functions that need to be called along with their arguments.`;

export const RETRIEVAL_TOOLS_PROMPT = `\n\n## Revtrieval Tools

Retrieval tools augment Assistant with knowledge from outside its model, such as proprietary product
information or documents provided by users.
...
sure to both read the beginning of the document using open and to make up to 3 searches to look
through later sections of the document.`;
